...
def soft_voting_by_n_best(data_dir, input_filenames, output_filename):
    '''
    parameter :
        data_dir (str) = "../outputs/test_dataset"
        input_filenames (list) = ["nbest_pred.json", "nbest_pred_9980.json"]
        output_filename (str) = "../outputs/test_dataset/ens.json"
    description :
        앙상블할 파일 목록을 입력하면, mrc_id에 따라 output_filename으로 파일 소프트보팅한 파일 생성 및 반환
    output : 
        output_filename (json 파일)
        output (pd.Series)
    '''
    ...
    for filename in input_filenames:
        df = pd.read_json(os.path.join(data_dir, filename))
        for k, topk in df.items():
            candidate = list(itertools.chain(*[[unit['text']] * (int(unit['probability']*10)+1) for unit in [*topk.values]]))
            mrc_dict[k].extend(candidate)
